...
from external_utils.approx.calculate_a_sa_ea_sets import initialize_a_sa_ea_tau_sets
from external_utils.approx.utilities import trace_to_list_of_str, concatenate_traces
from utils.new_approx_utils import add_trace_to_subtree, get_node_with_min_tree
...
def apply(obj: Union[Trace, EventLog], pt: ProcessTree, parameters=None):
    ...
    sublogs, pt = __align(obj, pt, max_trace_length=max_trace_length, max_process_tree_height=max_process_tree_height,
...
def __align(obj: Union[Trace, EventLog], pt: ProcessTree, max_trace_length: int = 1,
    ...
    sublogs, pt = __approximate_alignments_for_log(obj, pt, max_trace_length, max_process_tree_height,
                                            parameters=parameters)
    return sublogs, pt
...
def __approximate_alignments_for_log(log: EventLog, pt: ProcessTree, max_tl: int, max_th: int,
    ...
    for i, var in enumerate(variants):
        this_time = time.time()
        # if check_min_tree(pt):
        #     print("min tree found")

        # if this_time - log_alignment_start_time <= max_align_time:
        parameters["trace_alignment_start_time"] = this_time
        print('New variant:')
        sublog = __approximate_alignment_for_trace(pt, a_sets, sa_sets, ea_sets, tau_sets, log[variants[var][0]],
                                                      max_tl, max_th,
                                                      parameters=parameters)
        # alignment = add_fitness_and_cost_info_to_alignments(alignment, pt, log[variants[var][0]], parameters=parameters)
        # else:
        #     alignment = None
        # sublog.append(sublog)
        # add sublog number of times the variant occurs in the log
        if sublog is None:
            print('Process tree:', pt)
            variant_trace = log[variants[var][0]]
            # print concept names from log[variants[var][0]]
            print('Variant: ', [x['concept:name'] for x in variant_trace])
            traces = get_node_with_min_tree(pt)
            if traces is not None:
                for trace in traces:
                    # print('Trace: ', trace)
                    print('Trace: ', [x['concept:name'] for x in trace])
            # print(log[variants[var][0]])
            # get the traces from the subtree where
            print('Sublog is None')
            # print(log)
        # else:
            # print('Sublog is not None')
            # pass
        # print('Subtrace length: ', len(sublog))
        # print(type(sublog))
        for n in range(len(variants[var])):
            # print(type(sublogs))
            if sublog is not None:
                sublogs.append(sublog)
            else:
                # print('Sublogs is not a list or None')
                # print(type(sublogs))
                pass
...
def __approximate_alignment_for_trace(pt: ProcessTree, a_sets: Dict[ProcessTree, Set[str]],
    ...
    if parameters is None:
        parameters = {}
    add_trace_to_subtree(pt, trace)
    # Check if 'min_tree' property is set to True
    if pt._properties.get('min_tree'):
        # Return the current trace as a sublog immediately
        if trace is None:
            print('trace is None')
            # should be under properties traces
        return trace

    # if 'min_tree' in pt._properties:
    #     if pt._properties['min_tree']:
    #         # print('min_tree found', pt)
    #         if trace is None:
    #             print('trace is None')
    #         return trace
    ...
    if get_process_tree_height(pt) <= max_th or len(trace) == 0:
        # print('<max_tl <max_th for ', pt)
        return None
    else:
...
def __approximate_alignment_on_choice(pt: ProcessTree, trace: Trace, a_sets: Dict[ProcessTree, Set[str]],
                                      ...
                                      parameters=None):
    if pt._properties.get('min_tree'):
        # Return the current trace as the sublog
        print('Found min tree')
        return trace
    ...
    for j in range(len(pt.children)):
        ...
        if subtree._properties.get('min_tree'):
            min_tree_id = j
        mismatches = 0
        if len(trace) > 0:
        ...
        if mismatches < lowest_mismatches:
            ...
            best_id = j
    if min_tree_id is not None:
        print('Min tree in xor: ', pt.children[min_tree_id])
        if best_id == min_tree_id:
            return trace
        else:
            return None
...
def __approximate_alignment_on_loop(pt: ProcessTree, trace: Trace, a_sets: Dict[ProcessTree, Set[str]],
    ...
    from pulp import lpSum, LpVariable, LpProblem, LpMinimize, PULP_CBC_CMD
    ...
    status = ilp.solve(PULP_CBC_CMD(msg=0))
    assert status == 1
    ...
    for subtree, sub_trace in alignments_to_calculate:
        if subtree._properties.get('min_tree'):
            # Return the current trace as a sublog immediately
            print('Min tree in loop: ', subtree)
            if sub_trace is None:
                print('trace is None')
                # should be under properties traces
            return sub_trace
        subtrace = __approximate_alignment_for_trace(subtree, a_sets, sa_sets, ea_sets, tau_flags, sub_trace, tl, th,
                                              parameters=parameters)
        # if subtrace is None:
...
def __approximate_alignment_on_parallel(pt: ProcessTree, trace: Trace, a_sets: Dict[ProcessTree, Set[str]],
    ...
    for j in range(len(pt.children)):
        ...
        for trace_part in trace_parts:
            if subtree == trace_part[0]:
                ...
                if subtree._properties.get('min_tree'):
                    print('Min tree in parallel: ', subtree)
                    return sub_trace
        alignment = __approximate_alignment_for_trace(subtree, a_sets, sa_sets, ea_sets,
                                                                            tau_flags, sub_trace, tl, th,
                                                                            parameters=parameters)
        if alignment is not None:
            alignments_per_subtree[subtree] = alignment

    if not alignments_per_subtree:
        return None
        # if subtrace is not None:
        #     return subtrace
    ...
    for trace_part in trace_parts:
        ...
        # print('activities_to_cover: ', activities_to_cover)
        ...
        while activities_to_cover != activities_covered_so_far:
            ...
            move = alignment[0]
            alignment = Trace(alignment[1:])
            res.append(move)
            # print(move)
            # print(move[0])
            # if the alignment move is NOT a model move add activity to activities_covered_so_far
            if move['concept:name'] != SKIP:
                activities_covered_so_far.append(move['concept:name'])
