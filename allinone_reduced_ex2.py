...
def remove_events_by_name(event_log, event_name):
    new_event_log = copy.deepcopy(event_log)
    for trace in new_event_log:
        trace._list = [event for event in trace._list if event.get('concept:name') != event_name]
...
def remove_start_end_trans(net_in, im, fm):
    ...
    for transition in list(net.transitions):
        ...
        if (transition.label is None or transition.label == '') and (
                transition.name == 'start_tau' or transition.name == 'end_tau'):
            for arc in list(transition.in_arcs):
                if arc.source is source and len(transition.in_arcs) == 1 and len(transition.out_arcs) == 1 and len(
                        source.out_arcs) == 1:
                    # check if transition has one outarc
            ...
            for arc in transition.out_arcs:
                if arc.target is sink and len(transition.in_arcs) == 1 and len(transition.out_arcs) == 1 and len(
                        sink.in_arcs) == 1:
                    in_arc = next(iter(transition.in_arcs))
                    source_place = in_arc.source
                    # check if the only source of the one inarc also only has one inarc
                    if len(source_place.out_arcs) == 1:
                        ...
                        # remove arcs, transition, and place
                        remove_arc(net, arc)
...
def find_nets_with_net(net_in, t_add, log, pre_post_dict, skip, loop):
    ...
    tbc_nodes_inds_dict = extend_tbc_nodes_inds_dict(tbc_nodes_inds_dict_old, new_net, t_add, pre_post_dict)
    ...
    node_count_dict = {}
    print('Net size: ', len(net_in.transitions), len(net_in.places))
    print('New net size: ', len(new_net.transitions), len(new_net.places))
...
def subtree_process_presteps(net, im, fm, log, t_add, transitions, pre_post_dict, skip, loop):
    ...
    # root node of subtree has property min_subtree set True
    ...
    # here somehow a selfloop for start_tau and end_tau is created
...
def export_results(results, filename, threshold):
    ...
    resultfile = os.path.join('results', 'analysis', 'ex2', filename)
    print(df)
...
if __name__ == "__main__":
    ...
    i = 0
    for t in net.transitions:
        if t.label is not None and str(t.label) not in ['End', 'end', 'start', 'Start', 'START', 'END', 'Closed'] and str(t.label) == "O_Accepted":
            # print(t.label)
            ...
            pm4py.save_vis_petri_net(new_net_temp, new_net_temp_im, new_net_temp_fm,
                                     file_path=os.path.join('results', 'nets', 'ex2',
