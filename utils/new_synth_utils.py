        ...
        if new_t_skippable or new_t_in_loop:
            ...
            if new_t_in_loop:
                ...
                updated_incidence_mat, updated_places_dict, updated_trans_dict = apply_abstraction_rule(
                    ...
                    new_t_in_loop=False)
                updated_incidence_mat = np.concatenate((updated_incidence_mat,
                                                        -updated_incidence_mat[:,
                                                         updated_trans_dict[new_t_name]].reshape(-1, 1)), axis=1)
...
def apply_LD_p_then_abs_rules(incidence_mat, new_t_name: str, new_p_row,
    ...
    final_incidence_mat, final_places_dict, final_trans_dict = apply_abstraction_rule(
        incidence_mat=updated_incidence_mat,
    ...
    if new_t_skippable or new_t_in_loop:
        ...
        if new_t_in_loop:
            ...
            updated_incidence_mat, updated_places_dict, updated_trans_dict = apply_new_rule(
                ...
                new_t_in_loop=False)
            updated_incidence_mat = np.concatenate((updated_incidence_mat,
                                                    -updated_incidence_mat[:, updated_trans_dict[new_t_name]].reshape(
                                                        -1, 1)), axis=1)
...
def get_nodes_on_the_path(net, source_node_names: set, target_node_names: set,
    ...
    for source_target_pair in product(source_node_names, target_node_names):
        ...
        for node, pn_node in G_label.items():
            if type(pn_node) == pm4py.objects.petri_net.obj.PetriNet.Transition and pn_node.name == source_node_name:
                print(node)
        ...
        source_in_G_list = [node for node, pn_node in G_label.items() if type(
            pn_node) == pm4py.objects.petri_net.obj.PetriNet.Transition and pn_node.name == source_node_name]
        if len(source_in_G_list) > 0:
            source_in_G = source_in_G_list[0]
        else:
            print(f"No source node found in G_label with name {source_node_name}")
            continue  # or handle this error appropriately
        # target_in_G = [node for node, pn_node in G_label.items() if type(
        #     pn_node) == pm4py.objects.petri_net.obj.PetriNet.Transition and pn_node.name == target_node_name][0]
        target_in_G_list = [node for node, pn_node in G_label.items() if type(
            pn_node) == pm4py.objects.petri_net.obj.PetriNet.Transition and pn_node.name == target_node_name]
        if len(target_in_G_list) > 0:
            target_in_G = target_in_G_list[0]
        else:
            print(f"No source node found in G_label with name {target_node_name}")
            continue  # or handle this error appropriately
...
def get_tbc_nodes_inds_dict(net, new_t_name: str, log, t_Rs=0.6, noise_threshold=0.2):
    ...
    adjacent_act_dict_0_noise = get_adjacent_activities(new_t_name=new_t_name, caus_rel_dict=caus_rel_dict_0_noise,
                                                        t_Rs=t_Rs)
...
def get_the_best_net(net, new_t_name, log, net_dict=None, theta=0.9, visualize=False,
    ...
    new_nets_list = get_all_new_nets(net=net, new_t_name=new_t_name, tbc_nodes_inds_dict=tbc_nodes_inds_dict,
    ...
    for net_dict in new_nets_list:
        ...
        if not check_soundness.check_easy_soundness_of_wfnet(break_sc_petri):
            print('Net not sound')
    ...
    if len(cand_nets_dict) > 1:
        ...
        for cand_net_dict in cand_nets_dict:
            ...
            if use_recall:
                cand_net_dict['F1'] = 2 * (
                            (precision * cand_net_dict['recall']) / (precision + cand_net_dict['recall']))
            else:
    ...
    the_chosen_net_dict['n_nodes'] = len(the_chosen_net_dict['trans_dict']) + len(
        the_chosen_net_dict['places_dict']) - 1
...
def get_DFG_from_petri(net, im):
    ...
    tran_names = [re.search("(?<=').*(?='\))|None(?=\))", t.name).group() for t in rg.transitions if
                  'None' not in t.name]
...
def get_similarity_score(net, im, log, noise_threshold=0.2):
    ...
    nodes_in_DFG_from_log = set(
        [n for n in set(nodes_in_DFG_from_log) if len([e for e in DFG_from_log if e[1] == n]) >= 1]).union(
    ...
    preset_threshold_dict = {a: max([v for k, v in DFG_from_log.items() if k[1] == a]) * noise_threshold for a in
                             set([k[1] for k in DFG_from_log])}
    for k, v in start_activities.items():
        preset_threshold_dict[k] = v * noise_threshold

    postset_threshold_dict = {a: max([v for k, v in DFG_from_log.items() if k[0] == a]) * noise_threshold for a in
                              set([k[0] for k in DFG_from_log])}
    for k, v in end_activities.items():
    ...
    for node in nodes_intersect:
        pre_post_dict_log[node] = dict(
            preset=set([k[0] for k, v in DFG_from_log.items() if (k[1] == node) and (v > preset_threshold_dict[node])]),
            postset=set(
                [k[1] for k, v in DFG_from_log.items() if (k[0] == node) and (v > preset_threshold_dict[node])]))
    ...
    for node in nodes_intersect:
        ...
        len_postset_intersect = len(
            pre_post_dict_petri[node]['postset'].intersection(pre_post_dict_log[node]['postset']))
        len_postset_union = len(pre_post_dict_petri[node]['postset'].union(pre_post_dict_log[node]['postset']))

        similarity_score_dict[node] = (1 / 2) * (len_preset_intersect / len_preset_union) + (1 / 2) * (
                    len_postset_intersect / len_postset_union)
...
def toggle_sct(net, initial_marking, final_marking=None):
    ...
    if not is_sc:
        ...
        if 'short_circuited_transition' not in [t.name for t in net.transitions]:
            ...
            pnml_exporter.pnml.export_net(petrinet=new_net, marking=initial_marking, final_marking=final_marking,
                                          output_filename="temp_net.pnml")
...
def unblock_IM_model(net, im, fm, log, t_Rs=0.5, use_recall=False, tolerance=0.05, view_final_net=False,
                     noise_threshold=0.2,
                     return_DGs=True, metric_to_track='f1', return_net_dict=False, labels_tbi=None):
    quality_metrics_dict = dict(precision=[], fitness=[], recall=[], f1=[])
    fitness_dict = replay_fitness_evaluator.apply(log, net, im, fm,
                                                  variant=replay_fitness_evaluator.Variants.ALIGNMENT_BASED)
    ...
    initial_net_dict = dict(break_sc_petri=net, im=im, fm=fm, fitness_dict=fitness_dict, recall=recall,
    ...
    while True:
        ...
        if np.all(np.array([s for s in similarity_score_dict.values()]) == 1.0) or \
                ...
                (len(quality_metrics_dict[metric_to_track]) >= 3 and np.all(
                    np.diff(quality_metrics_dict[metric_to_track][-3:]) < 0)):
            break
        ...
        new_net, new_im, new_fm = toggle_sct(net=new_net, initial_marking=new_im)
        # pm4py.view_petri_net(new_net, new_im, new_fm)
        net_dict = get_the_best_net(net=new_net, new_t_name=tran_label, log=log, use_recall=False, theta=theta,
                                    t_Rs=t_Rs, noise_threshold=noise_threshold)
    ...
    if return_net_dict:
        return final_net_dict, quality_metrics_dict, intermediate_net_dicts
    else:
        return final_net_dict['break_sc_petri'], final_net_dict['im'], final_net_dict['fm']
